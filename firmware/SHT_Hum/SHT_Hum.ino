...
/* read the RH data from the sensor */
float readSTS21()
{
  Wire.begin();
  unsigned int data[2];
  ...
  double rh_value = -6.0 + 125.0 / 65536.0 * rawtmp;
  return rh_value;
}
/* reconnection */
void reconnect() {
...
void loop() {
  ...
  doc["hummidity"] = readSTS21();
  doc["batteryVoltage"] = readBatteryVoltage();
  doc["nodeId"] = readBatteryVoltage();
  ...
  doc["avalibleServices"] = "Hummidity(%RH),Battery Voltage(V)";
