 ...
 * RETURN:     -
 ******************************************************************************/
#define DS1820_DelayUs(dly_us)       delay(dly_us us)
 ...
 * RETURN:     -
 ******************************************************************************/
#define DS1820_DelayMs(dly_ms)   delay(dly_ms ms)
...
bool DS1820_Reset(void)
{
   ...
   clear_bit(OUT(TEMP_DATA_PORT),TEMP_DATA);
   ...
   DS1820_DelayUs(DS1820_PRESENCE_WAIT);
    clear_bit(DIR(TEMP_DATA_PORT),TEMP_DATA);
   /* get presence pulse */
   bPresPulse = ((P2IN & BIT4)== BIT4);
...
bool DS1820_ReadBit(void)
{
   ...
   bBit = ((P2IN & BIT4)== BIT4);//input(DS1820_DATAPIN);
   //printf(" read bit is %u \n",bBit);
   DS1820_EnableInterrupts();
 ...
 *
 * RETURN:     int         raw temperature value with a resolution
 *                            of 1/256?C
 ******************************************************************************/
int DS1820_GetTempRaw(void)
{
    uint8_t i;
    uint16_t temp_u16;
 ...
 * RETURN:     int         temperature value with an internal resolution
 *                            of TEMP_RES
 ******************************************************************************/
void DS1820_GetTempString(int tRaw_s16, char *strTemp_pc)
{
    int tPhyLow_s16;
    char tPhy_s8;

    /* convert from raw value (1/256?C resolution) to physical value */
    tPhy_s8 = (char)(tRaw_s16/TEMP_RES);
    ...
    sprintf(strTemp_pc, "%d.%02d", tPhy_s8, (char)tPhyLow_s16);
}
